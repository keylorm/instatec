...
<div class="page-content" ng-controller="proyectoExtensionesController" ng-cloak ng-init="proyecto_id='<?=$proyecto->proyecto_id?>'; consultarExtensionesProyecto();">
	        ...
	        <table class="table table-hover table-bordered" id="dataTable" width="100%" cellspacing="0">
	            ...
	            <tbody>
	            	<tr class="valor-oferta-item" ng-repeat="extension in extensiones  | limitTo : cantidad_mostrar : currentPage*cantidad_mostrar">
	            		<td  class="d-md-none"><a href="<?=base_url()?>proyectos/extensiones/<?=$proyecto->proyecto_id?>/editar-extension/{{extension.proyecto_valor_oferta_id}}" class="btn btn-sm btn-edit btn-success  mb-1"><i class="fa fa-fw fa-edit"></i></a> 
						...
						<td><a href="<?=base_url()?>proyectos/extensiones/<?=$proyecto->proyecto_id?>/editar-extension/{{extension.proyecto_valor_oferta_id}}">{{extension.fecha_registro}}</a></td>
						<td><a href="<?=base_url()?>proyectos/extensiones/<?=$proyecto->proyecto_id?>/editar-extension/{{extension.proyecto_valor_oferta_id}}">{{extension.proyecto_valor_oferta_extension_tipo}}</a></td>
						<td><a href="<?=base_url()?>proyectos/extensiones/<?=$proyecto->proyecto_id?>/editar-extension/{{extension.proyecto_valor_oferta_id}}">{{extension.valor_oferta | currency }}</a></td>
						<td  class="d-none d-md-table-cell"><a href="<?=base_url()?>proyectos/extensiones/<?=$proyecto->proyecto_id?>/editar-extension/{{extension.proyecto_valor_oferta_id}}" class="btn btn-sm btn-edit btn-success  mb-1"><i class="fa fa-fw fa-edit"></i></a> 
							<?php if(isset($rol_id) && $rol_id==1){ ?>
	...
	<p ng-if="extensiones===false" class="text-center table-espaciado">No hay ordenes de cambio registradas aún<br>
		<a class="btn btn-primary mt-4" href="<?=base_url()?>proyectos/extensiones/<?=$proyecto->proyecto_id?>/agregar-extension" role="button"><i class="fa fa-fw fa-plus-circle"></i> Agregar orden de cambio</a>
	</p>
