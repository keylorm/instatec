...
class M_Colaborador extends CI_Model {
	...
	function consultaAll($data = null){
		...
		if($result_colaborador->num_rows()>0){
			$result = array(
							...
							'datos' => $this->security->xss_clean($result_colaborador->result_array()),
							);
	...
	function getAllActiveColaboradores(){
		...
		if($colaborador_result->num_rows() > 0){
			return $this->security->xss_clean($colaborador_result->result_array());
		}else{
			return false;
		}
	}
	...
	function consultar($colaborador_id){
		if($colaborador_id!=null){
			...
			$result_colaborador = $this->db->get($this->t_colaborador);
			if($result_colaborador->num_rows()> 0){
				$result = $this->security->xss_clean($result_colaborador->row_array());
	...
	function consultaAllPuestos(){
		...
		if($result_puestos->num_rows()>0){
			$result =  $this->security->xss_clean($result_puestos->result_array());
			return $result;
	...
	function consultaPuestos($data = null){
		...
		if($result_colaborador_puestos->num_rows()>0){
			$result = array(
							...
							'datos' => $this->security->xss_clean($result_colaborador_puestos->result_array()),
							);
	...
	function getAllActiveJefesProyectos(){
		...
		$result_jefes_rows = $result_jefes->result_array();
		$result_jefes_count = $result_jefes->num_rows();
		if($result_jefes_count > 0){
			return $this->security->xss_clean($result_jefes_rows);
		}else{
			return false;
		}
	}
	...
	function consultarUsuarioIDJefeProyecto($jefe_proyecto_id) {
		$this->db->where('colaborador_id', $jefe_proyecto_id);
		$usuario_result = $this->db->get($this->t_usuario_colaborador);
		$usuario_result_count = $usuario_result->num_rows();
		if ($usuario_result_count > 0) {
			$usuario_result_row = $usuario_result->row_array();
			return $usuario_result_row['usuario_id'];
		} else {
			return false;
		}
	}
	...
	function consultarJefeProyectoIdUsuario($usuario_id) {
		$this->db->where('usuario_id', $usuario_id);
		$usuario_result = $this->db->get($this->t_usuario_colaborador);
		$usuario_result_count = $usuario_result->num_rows();
		if ($usuario_result_count > 0) {
			$usuario_result_row = $usuario_result->row_array();
			return $usuario_result_row['colaborador_id'];
		} else {
			return false;
		}
	}
