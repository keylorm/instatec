...
class M_Material extends CI_Model {
	...
	function consultarMaterial($material_id){
		if($material_id!=null){
			...
			if($result_material->num_rows()> 0){
				$result = $this->security->xss_clean($result_material->row_array());
				return $result;
			}else{
				return false;
			}
	...
	function getAllActiveMateriales() {
		...
		if ($result_materiales_count > 0) {
			return $this->security->xss_clean($result_materiales->result_array());
		}else{
			return false;
		}
    ...
    function consultaAllMaterialUnidades(){
		...
		if($result_unidades->num_rows()>0){
			$result =  $this->security->xss_clean($result_unidades->result_array());
			return $result;
	...
	function consultarMaterialUnidades($data = null){
		...
		if($result_material_unidades->num_rows()>0){
			$result = array(
							...
							'datos' => $this->security->xss_clean($result_material_unidades->result_array()),
							);
	...
	function consultaMaterialUnidad($material_unidad_id){
		if($material_unidad_id!=null){
			...
			if($result_unidad->num_rows()> 0){
				$result = $this->security->xss_clean($result_unidad->row_array());
				return $result;
			}else{
				return false;
			}
