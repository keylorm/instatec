...
<ol class="breadcrumb">
	...
	<li class="breadcrumb-item active">Horas por proyecto</li>
</ol>

<h1 class="text-center"><i class="fa fa-fw fa-table"></i> Reporte de horas por proyecto</h1>
<hr>


<div class="page-content" ng-controller="reporteHorasPorProyectoController">
	<div class="row">
		<div class="col-12"><h3 class=" text-center text-md-left"><i class="fa fa-fw fa-table"></i> Horas por proyecto</h3></div>		
	</div>
	<div class="card">
	        ...
	        <table class="table table-hover table-bordered" id="dataTable" width="100%" cellspacing="0">
	            ...
	            <tbody>
					<tr ng-repeat="proyecto in proyectos | limitTo : cantidad_mostrar : currentPage*cantidad_mostrar">
						<td class="d-md-none">
							<a href="<?=base_url()?>reportes/horas-por-proyecto/{{proyecto.proyecto_id}}" class="btn btn-edit btn-sm mb-1 btn-success" ><i class="fa fa-fw fa-list"></i></a>	
						</td>
						<td><a href="<?=base_url()?>reportes/horas-por-proyecto/{{proyecto.proyecto_id}}"><strong>Nombre: {{proyecto.nombre_proyecto}}</strong><br>N° Contrato: {{proyecto.numero_contrato}}<br>Orden de compra: {{proyecto.orden_compra}}</a></td>
						<td><a href="<?=base_url()?>reportes/horas-por-proyecto/{{proyecto.proyecto_id}}">{{proyecto.nombre_cliente}}</a></td>
						<?php if($rol_id==1){ ?>
							<td><a href="<?=base_url()?>reportes/horas-por-proyecto/{{proyecto.proyecto_id}}">{{proyecto.proyecto_estado}}</a></td>
							<td><a href="<?=base_url()?>reportes/horas-por-proyecto/{{proyecto.proyecto_id}}">Provincia: {{proyecto.provincia}}<br>Cantón: {{proyecto.canton}}<br>Distrito: {{proyecto.distrito}}</a></td>
							<td><a href="<?=base_url()?>reportes/horas-por-proyecto/{{proyecto.proyecto_id}}">Firma de contrato: {{proyecto.fecha_firma_contrato}}<br>Inicio: {{proyecto.fecha_inicio}}<br>Entrega: {{proyecto.fecha_entrega_estimada}}</a></td>
						<?php } ?>
